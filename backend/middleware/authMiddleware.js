...
function authMiddleware(req, res, next) {
  ...
  console.log('🔍 Auth Debug - Authorization header:', authHeader?.substring(0, 50) + '...');
  ...
  if (token.startsWith("test_admin_token_")) {
    ...
    User.findOne({ email: "testadmin@example.com", role: "admin" })
      ...
      .catch(err => {
        console.log('❌ Auth Debug - Server error during test authentication:', err.message);
  ...
  try {
    ...
    console.log('✅ Auth Debug - JWT verification successful, user ID:', decoded.id, 'role:', decoded.role);
...
function requireRole(roles) {
  ...
  return (req, res, next) => {
    ...
    if (!req.user || !allowed.includes(req.user.role)) {
      console.log('❌ Role Debug - Access forbidden. User:', req.user?.role, 'Required:', allowed);
