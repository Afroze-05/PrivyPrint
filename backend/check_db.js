...
mongoose.connect('mongodb://localhost:27017/privyprint')
  .then(async () => {
    ...
    recentLogs.forEach(log => {
      console.log(`  - ${log.time}: token=${log.token}, type=${log.printType}, price=${log.price}`);
    });
    ...
    recentDocs.forEach(doc => {
      console.log(`  - ${doc.createdAt}: token=${doc.token}, status=${doc.status}, type=${doc.type}, price=${doc.price}`);
    });
